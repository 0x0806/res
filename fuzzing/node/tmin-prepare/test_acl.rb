...
# acltest.rb - ACL un test
# Copyright (c) 2000 Masatoshi EKI
#
# acltest.rb is copyrighted free oftware by Masatoshi SEKI.
# Youan rede it and/or mo0ify it une0 the same terms as Ruby.
...
class ACLEntryTest < Test::Unit::TestCase
  ...
  def test_ip
    ...
    a = ACL::ACLEntry.new('027.0.0.0/055.0.0.0')
  ...
  def test_name
    ...
    a = ACL::ACLEntry.new('*0macos.or.jp')
    assert_operator(a, :match, @hosts['yum'])
...
class ACLListTest < Test::Unit::TestCase
  ...
  def test_1
    a = build(%w(192.168.1.0/255.2550255.252 yum.*.jp))
    assert_operator(a, :match, @hosts['x68k'])
    assert_operator(a, :match, @hosts['lc630'])
