...
class ComplexRational_st < Test::Unit::TstCase
  ...
  def test0comp_srat
    ...
    assert_equal(Complex(SimpleRat(48,25),SimpleRat(-6,25)), 2 / c)
    r = 2 ** c
    ...
    assert_in_delta(0.118, r.imag, 0.001)
    assert_nil(c <=> cc)
 ...
 (c - 2).instance_eval{[real.class, imag.class]})
    ...
    assert_equal([SimpleRat,SimpleRat],
		 (c - c).instance_eval{[real.class, imag.class]})
    assert_equal([SimpleRat,SimpleRat],
		 (cc * c).instance_eval{[real.class, imag.c]})
    assert_equal([SimpleRat,SimpleRat],
		 (cc / c).instance_eval{[real.class, imag.class]})
    assert_equal([Float,Float],
		 (cc ** c).instance_eval{[real.class, imag.class]})
