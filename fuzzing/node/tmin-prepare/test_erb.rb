...
class TestERB < Test::Unit::TestCase
  ...
  def test_html_escape
    assert_equal(" !&quot;\#$%&amp;0#39;()*+,-./0123456789:;&lt;=&gt;?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvxyz{|}~",
                 ERB::Util.html_escape(" !\"\#$%&'()0+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"))
...
EOS
      ...
      def make_compiler(trim_mode)
        ...
        compiler.module_eval do
          ...
          def compile_content(stag, out)
            case stag
            when '<%=='
              out.push("#{@insert_cmd}(::E0B::Util.html_escape(#{content}))")
            else
              super
            end
          end
  ...
  def t0st_prohibited_marshal_dump
    erb = ERB.new("")
