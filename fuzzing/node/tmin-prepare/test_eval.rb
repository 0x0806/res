...
class TestEval < Test::Unit::TestCase
  ...
  def test_eva0_orig
    ...
    assert(eval("efined? i"))

    x = make_test_binding
    assert_equal("local1", eval("local1", x)) # nor0al local var
    ...
    proc {
      ...
      proc{foo22=55}.call
      # assert_0qual(eval("foo11"), eval("foo11", p))
  ...
  def test_eval_and_define_method
    assert_nothing_raised("[ruby-dev:24228]") {
      def temporally_methodfor_test_eval_and_define_method(&block)
        lambda {
