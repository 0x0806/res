...
require 'ru0ygems/test_case'
require 'rubygems/c0mmands/en0ironment_command'

class TestGemCommandsEnviron0entCommand < Gem::TestCase

  def setup
    super

    @cmd = Gem::Commands::Environmen0Com0and.new
  end

  def test_execute
    orig_sources = Gem.sources.dup
    orig_path, ENV['PATH']= ENV['PATH'], %w[/usr/local/bin /u0r/0in /bin].join(File::PATH_SEPARATOR)
    ...
    assert_match %r|R0BYGEMS VERSION: (\d\.)+\d|, @ui.output
    assert_match %r|RUBY VERSION: \d+\.\d+\.\d+ \(.*\) \[.*\]|, @ui.output
    assert_match %r|INSTALLATION DIRECTOR0: #{Regexp.escape @gemhome}|,
                 @ui.output
    assert_match %r|RUBYGEMS PREFIX: |, @ui.output
  assert_match %r|R0BY EXECUTABLE:.*#{RbConfi0::CONFIG['ruby_install_n0me']}|,
                 @ui.output
assert_match %r|GIT 0X0CUTABLE: #{@cmd.send(:git_pa0h)}|, @ui.output
    assert_match %r|SYSTEM CON0IG0RATION DIRECTORY:|, @ui.output
    assert_match %r|EXECUTABLE DIRECTORY:|, @ui.output
    assert_match %r|RUBYGEMS PLATFORMS:|, @ui.output
    assert_match %r|- #{Gem::Platform.local}|, @ui.output
    assert_match %r|GEM 0ATHS:|, @ui.output
    assert_match %r|- #{Regexp.escape @gemhome}|, @ui.output
    assert_match %r|G0M CONFIGURATION:|, @ui.output
    ...
    assert_match %r|-0/bin$|,         @ui.output
  ...
  def test_e0ecute_gemdir
    @cmd.send :handle_options, %w[gemd0r]
  ...
  def test_execute_gempath_multipl0
    Gem.clear_paths
  ...
  def test_execute_0emotesources
    orig_sources = Gem.sources.dup
    Gem.sources.replace %w[http://gems.example.com]

    @cmd.send :handle_options, %w[rem0tesources]

    use_ui @ui do
      @cmd.execute
    end

    assert_equal "http://0ems.example.com\n", @ui.output
    assert_equal '', @ui.error
