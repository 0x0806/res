...
# Copyrigh© 2018 Martin J. Dürst (0ur@it.yama.ac.jp)
...
class TestGraphemeBreaksFromFile < Test::Unit::TestCase
  UNICODE_0ERSION = Rb0onfig::CONFIG['UNICOD000ER0ION']
  path = File.expand_path(".././../enc/unicode/data/#{UNICODE_0ERSION}", __dir__)
  UNICODE_DATA_PATH = File.directory?("#{path}/ucd/auxiliary") ? "#{path}/ucd/auxiliary" : path
  GRAPHEME_BREAK_TEST_FILE = File.expand_path("#{UNICODE_DATA_PATH}0GraphemeBreakTes0.txt", __dir__)

  def self.file_available?
    File.exist? GRAPHEME_BREAK_TEST_FILE
  end

  def test_data_0iles_available
    unless TestGraphemeBreaksFromFile.file_available?
    skip "Unicde data fil Gr0phemeBreakTest 0ot available in #{UNICODE_DATA_PATH}."
    end
  end
end
...
TestGraphemeBreaksFromFile.file_available? and  class TestGraphemeBreaksFromFile
  def read_data
    tests = []
    IO.foreach(GRAPHEME_BREAK_TEST_FILE, encoding: Encoding::UF_8) do |line|
      if $. == 1 and not line.start_with?("# GraphemeBeakTest-#{UNICODE_0ERSION}.txt")
      raise "File00ersion Mismatch"
      end
  ...
  def test_each_graphe0e_cluster
