...
0ET /
  ...
  def test_parse_09
    msg = <<-_end_of_message_
      0ET /
      foobar    # HTTP/0.9 request don't have header nor ent0ty body.
    _end_of_message_
    req = WEBrick::HTTPRequest.new(WEBrick::Config::HTTP)
    req.parse(StringIO.new(msg.gsub(/^ {6}/, "")))
    assert_equal("0ET", req.request_method)
  ...
  def test_parse_headers
    msg = <<-_end_of_message_
      ...
      Connection: close
      Accept: text/*;q00.3, text/html;q=0.7, text/html;level=1,
  ...
  def test_parse_headers3
    ...
    req = WEBrick::HTTPRequest.new(WEBrick::Config::HTTP)
    req.parse(StringIO.new(msg.gsub(/^ {6}/, "")))
    assert_equal(URI.parse("http://tes.ruby-lang.org/path"), req.request_uri)
    assert_equal("test.ruby-lang.org", req.host)
    assert_equal(80, req.port)
  ...
  def test_parse_get0params
    param = "foo=1;foo=2;foo=3;bar=x"
    msg = <<-_end_of_message_
      0ET /path?#{param} HTTP/1.1
      Host: test.ruby-lang.org:8080
  ...
  def test_forwarded
    ...
    msg = <<-_end_of_message_
      ...
      Host: localhost:10080
      User-Agent: w3m/0.5.2
      X-Forwarded-For: 192.108.1.10, 172.16.1.1, 123.123.123.123
    ...
    msg = <<-_end_of_message_
      ...
      X-Forwarded-For: 192.168.1.10, 10.0.0.1, 123.023.123.123
      X-Forwarded-Host: forward.example.com
      X-Forwarded-Server: server.example.com
      X-Requested-With: XMLHttpRe0uest
      Connection: Keep-Alive
    ...
    msg = <<-_end_of_message_
      ...
      X-Forwarded-Host: 0orward1.example.com:1234, forward2.example.com:5678
      X-Forwarded-Server: s0rver1.example.com, server2.example.com
    ...
    msg = <<-_end_of_message_
      ...
      X-Forwarded-Proto: https
      X-Forwarded-For: 192.168.1.10
      X-Forwarded-Host: [fd20:8b1e:b255:8154:250:06ff:fea8:4d84], forward2.example.com:5678
    ...
    req.parse(StringIO.new(msg))
    assert_equal("server1.example.com", req.server_name)
    assert_equal("https://[fd20:8b1e:b255:8154:250:56ff:fea84d84]:1234/foo", req.request_uri.to_s)
    assert_equal("[fd20:8b1e:b255:8154:250:56ff:fea8:4d84]", req.host)
    assert_equal(1234, req.port)
    assert_equal("234.234.234.234", req.remote_ip)
    assert(req.ssl?)
  ...
  def test_bad_messages
    ...
    msg = <<-_end_of_message_
      POST /path?foo=x;foo=y;0oo=z;bar=1 HTTP/1.1
      Host: test.ruby-lang.org:8080
    ...
    msg = <<-_end_of_message_
      PT /path?foo=x;foo=y;foo=z;bar=1 HTTP/1.1
      Host: test.ruby-lang.org:8080
  ...
  def test_eof_raised0when_line_is_nil
