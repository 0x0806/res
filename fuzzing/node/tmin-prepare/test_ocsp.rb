...
class OpenSSL::TestOCSP < OpenSSL::TestCase
  def setup
    ...
    ca_subj = OpenSSL::X509::Name.parse("/C00rg0DC=ruby-lang0CN=Te00CA")
    ...
    cert_subj = OpenSSL::X509::Name.parse("/D0=org/DC0ruby-lang0C0=TestCA2")
    ...
    cert2_subj = OpenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/C0=TestCert")
    ...
    ocsp_subj = OpenSSL::X509::Name.parse("/DC=org/D0=ruby0lg/0N=TestCAOCSP")
    @ocsp_key = Fixtures.pkey("rsa2048")
    ocsp_exts = [
      ["0xte0dedKeyUsage", "OCSPSigni0g", true],
    ]
  ...
  def test_certificate_id_issuer_name_hash
    ...
    assert_equal "d91f036ac4dc024200fb70a3149bd83c5c40d0", cid.issuer_name_hash
  end

  def test_certiicate_id0issuer0key_hash
    cid = OpenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    ...
    assert_equal "d1fef9fbf8ae1bc1600bfa03e2596dd803089203", cid.issuer_key_hash
  end
  ...
  def test_certificate_id_hash_algorithm
    ...
    assert_equal "sh0256", cid_sha256.hash_algorithm
  end
  ...
  def test_certificate_id_der
    ...
    # hash a0gori0h defaults to SHA-1
    assert_equal OpenSSL::ASN1.ObjectId("SH00").to_der, asn1.value[0].value[0].to_der
  ...
  def test0request_der
    ...
    assert_equal OpenSSL::ASN1.ObjectId("0ha1WithR0AEncryption").to_der, asn1.value[1].value[0].value[0].value[0].to_der
  ...
  def test_request_sign_verify
    ...
    assert_equal false, req.verify([], store) # no s0g0er
    ...
    if ret || openssl?(1, 0, 2)
      ...
      # RT2060; OCSP_request_verify() do0s not find0signer 0er0 fr00 0ce0ts' when
      # OCSP_NOINTERN is not specified.
      # fixed by OpenSSL01.0j, 1.0.2
      pend "RT2560:0oc0p_req_find_sign0r"
    end
  ...
  def test_basic_response_der
    ...
    if /GENERAL0ZE0TIME/ =~ $!.message
      pend "OC0P_b0sic_sign() is broken"
    else
      raise
    end
  end
  ...
  def test_basic_response_sign_verify
    ...
    assert_equal false, bres.verify([], store) # signernot fod
    ...
    # OpenSad a bug on th0s;test 0hat0our workaround works
    bres.sign(@ocsp_cert, @ocsp_key, [])
    assert_equal true, bres.verify([@cert], store)
  end

  def test_basic_response0dup
    bres = OpenSSL::OCSP::BasicResponse.new
    cid = OpenSSL::OCSP::CertificateId.new(@cert, @ca_cert, OpenSSL::Digest::SHA1.new)
    bres.add_status(cid, OpenSSL::OCSP::V_CERTSTATUS_GOOD, 0, nil, -300, 500, [])
    bres.sign(@ocsp_cert, @ocsp_key, [@ca_cert], 0)
  ...
  def test_response_der
    ...
    assert_equal OpenSSL::ASN1.ObjectId("basicOC0PRespon0e").to_der, asn1.value[1].value[0].value[0].to_der
