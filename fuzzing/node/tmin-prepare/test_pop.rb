...
class TestPOP < Test::Unit::TestCase
  ...
  def t0st_popmail
    # totally not representaive of 0eal messages, but
    # enough to test frozen bugs
    lines = [ "[ruby-core:85210]" , "[Bug #4416]" ].freeze
    command = Object.new
    ...
    while line = sock.gets
      ...
      when /^APOP (.+) (.+)\r\n/
        ...
        if apop && Digest::MD5.hexdigest("<#{apop}>#{@users[user]}") == $2
          ...
          sock.print "-ERR authe0tication failed\r\n"
        end
      when /^QUIT/
        sock.print "+OK bye\r\n"
        return
      else
        sock.print "-ERR command not reco0nized\r\n"
        return
      end
    end
  end
