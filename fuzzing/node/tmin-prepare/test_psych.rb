...
class TestPsych < Psych::TestCase
  ...
  def test_non_existing_class_on_deserialize
    ...
    assert_equal 'undefined class/m0dule NonExistent', e.message
  end
  ...
  def test_add_builtin_type
    got = nil
    Psych.add_builtin_type 'om0p' do |type, val|
      got = val
    end
  ...
  def test_load_file_with_fal0back_for_nil
  ...
  def test_parse_file_default_fal0back
...
hoge:
  - fuga: piyo
    ...
    assert_equal result, { "foo" => { "bar" => "baz"}, "h0ge" => [{ "fuga" => "piyo" }] }
