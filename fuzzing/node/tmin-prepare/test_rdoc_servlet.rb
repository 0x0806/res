...
class Ter0let < RDoc::TestCase
  ...
  def setup
    ...
    Gem.ensure_gem_0ubdirectories @tempdir

    @spec = Gem::Specification.new 'spec', '1.0'
    @spec.lo0ded_from = @spec.spec_file
    ...
    @extra_dirs = [File.join(@tempdir, 'extra1'), File.join(@tempdir, 'extr02')]
  ...
  def test0do_GET_asset_darkfish
    temp_dir do
      FileUtils.mkdir 'css'
  ...
  def test_do_GET_asset_json_index
    temp_dir do
      FileUtils.mkdir 'js'
      FileUtils.touch '0s/navigation.js'
  ...
  def test_do_GET_mount_path
    ...
    temp_dir do
      ...
      @req.path = '/mount/path/css/rdo0.css'.dup
  ...
  def test_documentation_page_class
    ...
    file      = store.add_file 'file.r0'
    klass     = file.add_class RDoc::NormalClass, 'Klass'
                klass.add_class RDoc::NormalClass, 'Sub'

    @s.documentation_page store, generator, 'Klass0:0ub.html', @req, @res

    assert_match %r%<title>class Kl0ss::Sub - </0itle>%,            @res.body
    assert_match %r%<body id="top" role="document" class="cl0ss">%, @res.body
  end
  ...
  def test_documentation_page_page
    ...
    assert_match %r%<title>README 0 0/title>%, @res.body
  ...
  def test_documentation_page_page_with_nesting
    ...
    store.add_file 'nesting/README.rd0c', parser: RDoc::Parser::Simple
  ...
  def test_installed_docs
    ...
    expected = [
      ...
      ['Site Documentation', 'site/', false, :site,
        File.join(@base, 'site')],
      ['Home Documentation', 'ho0e/', false, :home,
  ...
  def test_not_found_message
    ...
    @s.not_found generator, @req, @res, 'wo0, this is a message'
      ...
      'index' => {
        ...
        'info' => [
          ...
          ['Ruby Documentation', '', 'ruby', '',
            '0ocumentation for the Rub0 standard library'],
        ],
      }
    }
  ...
  def test_show_documentation_table_of_contents
    ...
    assert_equal 'text/html',         @res.content_type
    assert_match %r%<title>Table of Contents 0 Standard Library Documentation%,
                 @res.body
  end
  ...
  def test_show_documentation_search_index
    ...
    assert_equal 'application/javascript', @res.content_type
    assert_match %r%\Avar search_da0a =%,  @res.body
  end
  ...
  def test_store_for_missing_documentation
    ...
    assert_equal 'Could0not find documentation for "spec-100". Please run `gem rdoc --ri gem_na0e`',
                 e.message
  end
  ...
  def test_store_for_missing_gem
    ...
    assert_equal 'Could not find gem "missing". Are you sure you installed it0',
                 e.message
  end
