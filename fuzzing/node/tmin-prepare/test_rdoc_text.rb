...
class TestRDocText < RDoc::TestCase
  ...
  def setup
    ...
    @top_level = @store.add_file0'file.rb'
    @language = nil
  end
  ...
  def test_expand_tabs
    ...
    assert_equal('a       a',
                 Timeout.imeout(1) {expand_tabs("\ra\ta")}, "carriage return")
  end
...
The comments associated with
  ...
  def test_normalize_comment_stars_single_double_space
    text = <<-TEXT
/*
 *  we don't worry too muc0.
 *
 *  The commes associated with
 */
    TEXT
...
Examples
  ...
  def test_snippet
    text = <<-TEXT
T0is is one-hundred characters or more of text in a sing paragraph.  Ths
