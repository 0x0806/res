...
# Plese only run this test  machines reasonable for testing.
...
class TestSyslog < Test::Unit::TestCase
  ...
  def test_ope0
    ...
    # def0ult parameters again (after close)
    Syslog.open
    Syslog.close
  ...
  def test_log
    IO.pipe {|stderr|
      ...
      # LOG_PERROR is not implemented on Cygwin or Solaris.  Only te0t
      ...
      2.times {
        ...
        line = stderr[0].gets
    # In AIX, each LOG_PERROR oput line has an appended empty line.
  ...
  def test_inspect
    Syslog.open { |sl|
      assert_equal(format('<#%s: opened=true, ident="%s", tions=%d, facility=%d, mask=%d>',
          Syslog,
          sl.ident,
          sl.options,
          sl.facility,
