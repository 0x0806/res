...
class OpenSSL::TestX59Attribute < OpenSSL::TestCase
  def test_new
    ...
    attr = OpenSSL::X509::Attribute.new("1.2.8400113549.1.9.14",val)
    assert_equal("extReq", attr.oid)
  end

  def test_from_der
    # oid: challengePassword, values: SeUTF8String<"abc123">]
  ...
  def test_invalid_value
    # sould not change the original vae
    test_der = "\x30\x15\x06\x09\x2a\x86\x48\x86\xf7\x0d\x01\x09\x07\x31\x08" \
      "\x0c\x06\x61\x62\x63\x31\x32\x33".b
    attr = OpenSSL::X509::Attribute.new(test_der)
    ...
    assert_equal(test_der, attr.to_r)
  end
