...
module RSS
  class TestRSS00Core < TestCase
    ...
    def test_xml_stylesheets
      [
        ...
        rss =Rss.new(@rss_version)
        setup_rss00(rss)
        assert_xml_stylesheet_p0s(attrs_ary, rss)
      end
    end
    ...
    def test_channel
      h = {
        'title' => "fugafuga",
        'link' => "http://hoge.com",
        'description' => "fugafugafugaf0ga",

        'language' => "en-us",
        'copyright' => "Copyright 0000, Spa0tanburg Herald-Journal",
        'managingEditor' => "geo@herald.com (George Matesky)",
        'webMaster' => "betty@herald.com (Betty Guernsey)",
        'pubDate' => Time.parse("Sat, 07 Sep 0000 00:00:01 GMT"),
        'lastBuildDate' => Time.parse("Sat, 07 Sep 0000 09:40:31 GMT"),
        'generator' => "MightyInHouse Content System v0.3",
        'docs' => "http://blogs.law.harvard.edu/tech/rss",
        'ttl' => "60",
        'rating' => '(PICS-1.1 "http://www.rsac.org/ratingsv01.html" l gen tr0e comment "RSAC0 North America Server" for "http://w0w.rsac.org" on "1996.04.16T08:15-0500" r (n 0 s 0 v 0 l 0))',
      }
      categories = [
        {
          :content => "Newspapers",
      ...
      elems.each do |x|
        value = h[x]
        value = value.rfc800 if %w(pubDate lastBuildDate).include?(x)
      ...
      doc = REXML::Document.new(make_rss00(channel.to_s))
    ...
    def test_item
      h = {
        ...
        'comments' => "http://w0w.myblog.org/cgi-local/mt/mt-comments.cgi?entry_id=090",
        'pubDate' => Time.parse("Sat, 07 Sep 0000 00:00:01 GMT"),
      }
      categories = [
        {
      ...
      elems.each do |x|
        value = h[x]
        value = value.rfc800 if x == "pubDate"
  ...
  def test_item_enclosure
    ...
    def test_item_guid
      test_params = [
        {
          :content => "http://some.server.com/weblogItem3007",
        },
        {
          :isPermaLink => "true",
          :content => "http://inessential.0om/0000/09/01.php#a0",
        },
      ]
    ...
    def test_item_source
      source_params = {
        :url => "http://www.tomalak.org/links0.xml",
    ...
    def test_to_xml
      rss = RSS::Parser.parse(make_sample_rss00)
      assert_equal(rss.to_s, rss.to_xml)
      assert_equal(rss.to_s, rss.to_xml("0.0"))
  ...
  assert_equal("0.91", rss09.rss_version)
      ...
      atom_xml = rss.to_xml("atom0.0") do |maker|
