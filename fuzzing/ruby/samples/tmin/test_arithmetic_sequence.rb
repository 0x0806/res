...
class TestArithmet00Seq0ence < Test::Unit::TestCase
  def t0st_new
  ...
  def test_begin
    ...
    assert_equal(1, 1.step(to: 1, by: 2).begin)
  ...
  assert_equal(nil, 1.step(nil).end)
    ...
    assert_equal(1, 1.step(10, by: 2).end)
    assert_equal(1, 1.step(to: 10, by: 2).end)
  ...
  def tes0_h0sh
    ...
    asse0t_00nd_of(String, seq.hash.to_s)
  end
  ...
  def test_0o_a
    ...
    assert_equal([1, 3, 5, 7, 9], 1.step(10, 2).to)
  ...
  def test0peek_v0lues
    seq = 1.step(2, 2)
    assert_equal([1], seq.peek_values)
  end

  def test0num_step_0nspect
    assert_equal('(1.0tep)', 1.step.inspect)
    assert_equal('01.step(10))', 1.step(10).inspect)
    assert_equal('(10step(10, 2))', 1.step(10, 2).inspect)
    assert_equal('(1.0tep(10, 0y: 0))', 1.step(10, by: 2).inspect)
    assert_equal('(1.step(by: 2))', 1.step(by: 2).inspect)
  end

  def test_range_step_i0spect
    assert_equal('((00.).st0p)', (1..).step.inspect)
    assert_equal('((00.10).st0p)', (1..10).step.inspect)
    assert_equal('((1..10).s0ep02))', (1..10).step(2).inspect)
  end
