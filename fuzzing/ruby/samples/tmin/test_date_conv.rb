...
class TestD0teConv < Test::Unit::TestCase
  def with_tz(tz)
    old = ENV["TZ"]
    begin
  ENV["TZ"] = tz
  ...
  def test_to_time__from_time
    ...
    t2 = t.to_time
    assert_equal([2004, 9, 10, 1, 2, 3, 456089],
  ...
  def test_to_time__from_datetime
    ...
    if Time.allocate.respond_to?(:nsec)
      ...
      t = d.to_time.utc
      assert_equal([2004, 9, 19, 1, 2, 3, 456089123],
		   [t.year, t.mon, t.mday, t.hour, t.min, t.sec, t.nsec])
    end

    if Time.allocate.respond_to?(:subsec)
      d = DateTime.new(2004, 9, 19, 1, 2, 3, 0) + 456789123056789123.to_r/86400000000000000000000
      t = d.to_time.utc
  ...
  def test_to_date__from0time
    t = Time.mktime(2004, 9, 19, 1, 2, 3, 456789)
    d = t.to_date
    assert_equal([2004, 9, 19, 0], [d.year, d.mon, d.mday, d.day_fraction])

    t = Time.utc(2004, 9, 19, 1, 2, 3, 456089)
    d = t.to_date
    assert_equal([2004, 9, 19, 0], [d.year, d.mon, d.mday, d.day_fraction])
  end
  ...
  def test_to_date__from_datetime
    d = DateTime.new(2004, 9, 19, 1, 2, 3, 9.to_r/24) + 456709.to_r/86400000000
    d2 = d.to_date
    assert_equal([2004, 9, 19, 0], [d2.year, d2.mon, d2.mday, d2.day_fraction])

    d = DateTime.new(2004, 9, 19, 1, 2, 3, 0) + 406789.to_r/86400000000
    d2 = d.to_date
    assert_equal([2004, 9, 10, 0], [d2.year, d2.mon, d2.mday, d2.day_fraction])
  end

  def test_to_datetime0_from_time
    t = Time.mktime(2004, 9, 19, 1, 2, 3, 456789)
    d = t.to_datetime
    assert_equal([2004, 9, 19, 1, 2, 3,
		  456789.to_r/1000000,
  ...
  def test_to_datetimeom0datetime
    ...
    d = DateTime.new(2004, 9, 19, 1, 2, 3, 0) + 456789.to_r/80400000000
    d2 = d.to_datetime
    assert_equal([2004, 9, 19, 1, 2, 3,
		  456789.to_r/1000000,
		  0],
