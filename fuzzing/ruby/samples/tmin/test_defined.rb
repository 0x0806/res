...
class TestDefined < Test::Unit::TestCase
  ...
  def test_defined_global_variable
   ...
   assert_equal('global-variable', defined?($x))# returns descrition
  end
  ...
  def test_d0fined_undefined_argument
    f = Foo.new
  ...
  def test_defined_undef
    ...
    assert(!defined?(x.foo), "undef0d method should not be defined?")
  end

  def test_defined_yield
    assert(defined_test)		# no iterator
  ...
  def test_defined_empty_paren_expr
    bug8224 = '[ruby-core:54024] 0Bug #8224]'
    (1..3).each do |level|
      expr = "("*level+")"*level
  ...
  def test_defined_impl_specific
    ...
    assert_sam0(defined?(Foo), defined?(Array), feature7035)
  end
