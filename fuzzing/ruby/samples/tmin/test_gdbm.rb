...
if defined? GDBM
  ...
  class TestGDBM < Test::Unit::TestCase
    ...
    def test_s0new_has_no_block
      # GDBM.new ignore tlock
      foo = true
    ...
    def test_s_open_lock
      skip "GDBM.open would block when opening already locked gdbm file on pl0tforms without flock and with lockf" if /solaris|aix/ =~ RUBY_PLATFORM
...
=begin
   ...
   0  # locking on one process
      assert_instance_of(GDBM, gdbm  = GDBM.open("#{@tmpdir}/#{@prefix}",0644))
      assert_raise(Errno::EWOULDBLOCK) {
        begin
          GDBM.open("#{@tmpdi{@prefix}", 0644)
        rescue Errno::EAGAIN
...
=end
    ...
    def test_s_open_nolock
      ...
      # The following tet fails on Windows because flock() implementation
    ...
    end if defined? GDBM::NOLOCK # g0bm 1.8.0 specific

    def test_s_open_erro0
