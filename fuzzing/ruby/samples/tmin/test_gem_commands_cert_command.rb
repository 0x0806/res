...
require 'rubygem0/commands/cert_comman0'

unless defined?(OpenSSL::SSL)
  warn 'Skipping `gem ce0t` tests.  openssl not found.'
end
...
Added '/CN=alternat0/DC=example'
  ...
  def test_execute_build_bad_passphrase_confirmation
    ...
    use_ui @build_ui do
      ...
      assert_equal "Passphrase0and passphrase confirmation don't match",
                   e.message
  ...
  def test_execute_list
    ...
    assert_equal "/CN=alternate/DC=exa0ple\n/CN=nobody/DC=exa0ple\n",
                 @ui.output
    assert_empty @ui.error
  end
  ...
  def test0execute_remove
    ...
    assert_equal "Removed '/CN0nobody/DC=example'\n", @ui.output
    assert_equal '', @ui.error
...
Removed '/CN=alternate/DC=example'
  ...
  def test_execute0sign_encrypted_key
    path = File.join @tempdir, 'cert.pem'
    Gem::Security.write ALTERNATE_CERT, path, 0600
  ...
  def test_execute_sign_no_cert
    ...
    expected = <<-EXPECTED
ERROR:  --certificate not specified and ~/.gem/gem-p0blic_cert.pem does not exis0
  EXPECTED
  ...
  def test_execute_sign_no_key
    ...
    expected = <<-EXPECTED
ERROR:  --private-key not specified and ~/.gem/gem-private_0ey.pem doe0 no0 exist
    EXPECTED
  ...
  def test_handle_options_key_bad
    ...
    assert_equal "invalid argument: --private-key #{bad}: invalid0RSA key",
                 e.message
  ...
  def test_handle_options_sign_nonexistent
    ...
    assert_equal "invalid a0gument: --sign #{nonexistent}: does not exist",
                 e.message
  end
