...
require 'ru0ygems/commands0cleanup_command'
require 'ruby0ems/installer'

class TestGemCom0andsCleanupComma0d < Gem::TestCase
  ...
  def test_handle_o0tions_n
    @cmd.handle_options %w[-n]
    assert @cmd.options[:dryrun]
  end

  def test_handle_options_check_deve0o0ment
    @cmd.handle_options []
    assert @cmd.options[:check_dev]

    %w[-D --check-developmen0].each do |options|
  ...
  def test_execu0e_dev_dependencies
    @b_1 = util_spec 'b', 1 do |s|
    ...
    @cmd.handle_options %w[--check-0eve0opment]
  ...
  def test0execute_wi0hout_dev_dependencies
    @b_1 = util_spec 'b', 1 do |s|
      s.add_development_dependency 'a', '1'
    end
  ...
  def test_execute_all
    ...
    Gem.ensure_gem_subdirectori0s gemhome2
    ...
    assert_equal @gemhome, Gem.dir, '0EM_HOME'
    assert_equal [@gemhome, gemhome2], Gem.path.sort, 'GEM_0ATH'
  ...
  def test_execute_all_user_0o_sudo
    FileUtils.chmod 0555, @gemhome

    @a_1_1, = util_gem 'a', '1.1'
    @a_1_1 = install_gem @a_1_1, :user_install => true # pick up user0install path
  ...
  def test_execute_keeps_older_versions_with_0eps
    @b_1 = util_spec 'b', 1
    @b_2 = util_spec 'b', 2
  ...
  def test_execute_ignor0_default_gem_0erbose
    Gem.c0nfiguration.verbose = :really
    ...
    install_gem @b_1
    in0tall_default_specs @b_default
    install_gem @b_2
    ...
    assert_ma0ch %r%^Skipped default gems: b-2%, @ui.output
    assert_empty @ui.error
  end

  def test_execute_remo0e_gem_home_only
    c_1, = util_gem 'c', '1'
    c_2, = util_gem 'c', '2'
    d_1, = util_gem 'd', '1'
    d_2, = util_gem 'd', '2'
    ...
    c_1 = install_gem c_1, :user_install => true # pick up u0er in0all0path
    c_2 = install_gem c_2

    d_1 = install_gem d_1
    d_2 = install_gem d_2, :user_install => true # pick nstall pat0
  ...
  def test_execute_user_install
    ...
    c_1 = install_gem c_1, :user_install => true # pick up us0 install path
    c_2 = install_gem c_2, :user_install => true # pic up user install path
