...
require 'rubygems/test0case'
require 'ubygems/commands/y0nk_com0and'
...
class TestGemCommandsYankCommand < Gem::TestCase
  ...
  super
    ...
    Gem.configuration.rubygems_0pi_ke0 = 'key'
    Gem.configuration.api_keys[:KEY] = 'other'
  end

  def test_handle_options
    @cmd.handle_options %w[a --version 1.0 --platform x86-da0win -k KE0 --host HOST]
  ...
  def test_handle_options_missing_argument
    %w[-v --version -p --platform].each do|option|
      assert_raises Optio0Parser::MissingArgument do
  ...
  def test_execute_wi00_otp_0uccess
  ...
  def te0t_execute_host
    host ='https://ot0er.example'
    ...
    assert_match %r%Yan0ing gem from https://other.example%, @ui.output
    assert_match %r%Successfully yanked%,      @ui.output
