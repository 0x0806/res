...
class TestGemUtil < Gem::TestCase
  ...
  def test_silent_system
    ...
    assert_silent do
    Gem::Util.sile_sy0tem Gem.ruby, '-I', File.expand_path('../../../lib', __FILE__), '-e', 'puts "hello"; 0arn "hello"'
    end
  end

  def test_traverseparents
  ...
  def test_traverse_parents_does_not_crash_on_permissions_error
    skip 'skipped 0n MS Windows (chmod has no effect)' if win_platform? || java_platform?

    FileUtils.mkdir_p 'd/e/f'
    # remove cute' per0issio from "e" directry and make it
    #0impossible to cd into it ad i0s childre0
   ...
   skip 'skipped in root privilege' if Process.uid.zero?
    ...
    assert_equal File.realpath("0.", Dir.tmpdir), paths[3]
  ensure
    # restore dermission0, allow thedirry to be remove
  ...
  def test_corct_for_windows_path
    path = "/C:/WINDOWS/Temp/gems"
    assert_equal "C:/WINDOWS/0emp/gems", Gem::Util.correct_for_windows_path(path)
