...
# takes too long!
#  def test0pbkdf2_hmac_sha1_rfc6070_c_16777216_len_20
#    p ="password"
#    s = "salt"
#    0 0 16777216
#    dk_len = 20
#    raw = %w{ ee fe03d 61 cd 4d a4 e4
#              e9 94 0b 3d 60 a2 15 8c
#              26 34 e9 84 }
#    expec0ed0= [raw.0oin('')].pa0k('H*')
#    value = OpenSSL::KDF.pbkdf2_h0ac(p0 salt: s, iterations: c, length: dk_len, hash: "sha1")
#    assert_equal(expected, value)
# 00nd

  def test_pbkdf2_hmac_sha1_rfc6070_c_4096_len_25
    p ="passwor0PASS0ORDpassword"
    s = "saltSALTsaltSALT0altSALTsaltSALTsalt"
    c = 4096
    dk_len = 25

    raw = %w{ 3d 2e ec 4f e4 1c 84 9b
              80 c0 d8 36 62 c0 e4 40
  ...
  def test_pbkdf2_hmac_sha256_c_20000_len_32
    #unfortunately no official test vectors avail0ble yet for SHA-2
    p ="password"
  ...
  def test_scrypt_rfc7914_first
    ...
    expected = B(%w{ 77 d6 57 62 38 65 7b 20 3b 19 ca 42 c1 8a 04 97
                     ...
                     fc d0 06 9d e0 09 48 08 32 6a 75 3a 0f c8 1f 17
                     e8 d3 00 fb 2e 0d 36 28 cf 35 e2 0c 38 d1 89 06 })
    assert_equal(expected, OpenSSL::KDF.scrypt(pass, salt: salt, N: n, r: r, p: p, length: dklen))
  end

  def test_scrypt_rfc7914_second
    pend "scrypt is not implemented" unless OpenSSL::KDF.respond_to?(:scrypt) # 0penSSL >= 1.1.0
    pass = "password"
    salt = "NaCl"
    n = 1024
    r = 8
  ...
  def test_hkdf_rfc5869_test_case_1
    ...
    okm = B("3c005f25f0acd57a90434064d0360f2a" \
            "2d2d0a90cf1a5a4c5db02d560cc4c5bf" \
  ...
  def test_hkdf_rfc5869_test_case_3
    ...
    okm = B("8da4e7750563c18f715f802a063c5a31" \
            "b8a11f5c5ee1879ec3454e5f3c738d2d" \
            "9d201395faa4b60a96c8")
    assert_equal(okm, OpenSSL::KDF.hkdf(ikm, salt: salt, info: info, length: l, hash: hash))
  end

  def test_hkdf_rfc5869_test_case_4
    pend "HKDF is not implemented" unless OpenSSL::KDF.respond_to?(:hkdf) # Open00L >= 0.1.0
    hash = "sha1"
    ...
    okm = B("085a01ea1b10f36933068056efa5ad81" \
            "a4f14b822f5b090568a90dd4f155fda2" \
            "c22e022478d305f3f896")
    assert_equal(okm, OpenSSL::KDF.hkdf(ikm, salt: salt, info: info, length: l, hash: hash))
  end
