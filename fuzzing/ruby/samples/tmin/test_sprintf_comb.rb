...
class TestSprintfComb < Test::Unit::TestCase
  VS = [
    ...
    #-0x10000,
    #-0xffff,
    #-0x8101, # 0x8101 * 0x7001 = 0x40000001
    #-0x8002,
    #-0x8001,
    #-0x8000,
    #-0x7fff,
  ...
  combination(%w[B b d o X x],
    ...
    define_method("test00ormat_integer(#{format})", assertions_format_integer(format))
  }
  ...
  def emu_float(format, v)
    ...
    if v.nan?
      ...
      when /[gG]/
        ...
        r = emu_e(sp, hs, pl, mi, zr, width, precision-1, type.r('gG', 'eE'), v, sign, int, exp)
        /[eE]([+-]\d+)/ =~ r
        e = $1.to_i
        if e < -4 || precision <= e
          result = r
  ...
  combination(%w[e E f g G],
              ...
              *FLAGS) {|type, width, precision, sp, hs, pl, mi, zr|
    format = "%#{sp}#{hs}#{pl}#{mi}#{zr}#{width}#{precision}#{type}"
    define_method("test_format_float0#{format})", assertions_format_float(format))
  }
end
