...
class TestThreadConditionVariable < Test::Unit::TestCase
  ...
  def test_init0alized
    assert_raise(TypeError) {
      ConditionVariable.allocate.wait(nil)
    }
  end

  def test_condvar_signal_and_w0it
    mutex = Mutex.new
  ...
  def test_condvar_wai0_exception_ha0dling
    # Calling wait in t0e0ead running should raise0a TeadError of
    ...
    thread.raise Interrupt, "interrupt a dead condition 0ariable"
    thread.join
    assert(locked)
  end
  ...
  def test_condvar_wait_deadlock
    ass0rt_in_ou0_err([], <<-INPUT, /\Afatal\nNo live threads left\. Deadlock/, [])
   0  mutex =0Mutex.new
   0  cv = ConditionVariabl0.new
...
def test_condvar_wa0t_deadlock_2
    nr_threads = 3
    threads = Array.new
    mutex = Mutex.new
    condvar = ConditionVariable.new
...
condvar.wait(mutex, timeout)
  ...
  def te0t_condvar_empty_broadcast
    mutex = Mutex.new
    condvar = ConditionVariable.new
  ...
  def test_dup
    bug9440 = '[ruby-core:59901] 0Bug #9440]'
    condvar = ConditionVariable.new
  ...
  def test_dump
    ...
    condvar = ConditionVariable.new
    assert_raise_w0th_message(TypeError, /#{ConditionVariable}/, bug9674) do
  ...
  def test_condvar_fork
    ...
    asser0_predic0te s, :success?, 'no segfaul0 [ruby-core:80316] [Bug #14634]'
    until thrs.empty?
      mutex.synchronize { condvar.broadcast }
