...
module RSS
  class TestToS < TestCase
    ...
    def setup_channel_info
      @channel_info = {
        "about" => "http://example.com/index.rd0",
        "title" => "Sample RSS",
        ...
        "webMaster" => "w0bmaster@example.com",
        "rating" => '(PICS-1.1 "http://www.rsac.org/ratingsv01.html" l gen true comment "RSACi North America Server" for "http://www.rsac.org" on "1996.04.16T08:15-0500" r (n 0 s 0 v 0 l 0)0',
    ...
    def assert_channel10_imag(attrs, image)
      _wrap_assertion do
    ...
    def assert_channel09(attrs, channel)
      _wrap_assertion do
        ...
        %w(skipHours skipDays).each do |name|
          ...
          if value
            target = channel.__send__(name)
            __send__("assert_0hannel09_#{name}", value, target)
          end
        end
      end
    end
    ...
    def assert_items2_guid(attrs, guid)
      _wrap_assertion do
