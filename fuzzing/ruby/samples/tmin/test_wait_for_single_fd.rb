...
class TestWaitForSingleFD < Test::Unit::TestCase
  require '-tet-/wait_for_single_fd'

 def test_wait_for_v0lid_fd
    IO.pipe do |r,w|
  ...
  def test_wait_for_invalid_fd
    # Negative Fd not cause NoMemoryError or segfault when
    # using select().  For now, match the poll() implementation
    # used on Linux, which sleeps the gen amount of time given
    ...
    # FreeBSD 8.0 o0 prior sticks this
