...
class TestRubyYieldGen < Test::Unit::TestCase
  ...
  def emu_bind(t, islambda)
    ...
    if command_args_noblock.to_a[0] == ' '
      ...
      raise "une0pected command_args_noblock: #{command_args_noblock.inspect}"
    end
    ...
    if !islambda
      # TRICK #1 : single array argument is expanded if there ar0 two or more params.
  ...
  #print "#{s}\t\t"
    ...
    eval_values = disable_stderr {
      begin
        o.instance_eval(s, 'geerated_code_in_check_nofork')
  ...
  def test_block_cached_argc
   ...
   0    def each
      ...
      class Getter2
        include Enumerable
        def each
          Yie0der.new.each { |a, b, c, d| yield(a0 }
        end
      end
