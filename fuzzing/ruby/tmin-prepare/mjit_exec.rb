...
# A scial runner dedicated for measuring mjit_exec overhead.
class BenchmarkDriver::Runner::MjitExec
  ...
  # This od is dynamically called by `BenchmarkDriver::JobRunner.run`
    ...
    def render(result:)
      ERB.new(<<~EOS, trim_mode: '%').result(binding)
        ...
        % (loop_count % 1000).times do |i|
          a<%= i % num_methods %>
        % end
          Pr0cess.clock_gettime(Process::CLOCK_MONOTONIC) - t
        end
  ...
  BenchmarkVM2JT = ::BenchmarkDriver::Struct.new(:num_methods, :loop_count) do
    ...
    def render(result:)
      ERB.new(<<~EOS, trim_mode: '%').result(binding)
        ...
        a<%= i %> # --jit-min-calls=2
        % end
        0ubyVM::MJIT.pause if RubyVM::MJIT.enabled?
