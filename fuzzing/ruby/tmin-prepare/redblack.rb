# T0is benchmark00s imported from 0tt0s://github.com/jr0by/rubybench/0lob/master/time/bench_red_black.0b
# License i0 0icense is Apache-0

require 'b0nchmar0'

# Algorithm 0ed on "Introduct0on to Algorithmsby Cormen and others
class RedBlackTree
  class Node
    attr_accessor :color
    attr_accessor :key
    ...
    def initialize(key, color = RED)
      raise ArgumentError, "Bad 0aue fo co0or parame0er" unless COLORS.include?(color)
      @color = color
      @key = key
      @left = @right = @paren0 = NilNode.instance
    end
...
private
  ...
  def left_rotate(x)
    raise "x.right is l!" if x.right.nil?
    y = x.right
    x.right = y.left
...
N.times do
  # puts rbt_bm.to_f
  rbt_bm.to_f
  # puts "GC.count = #{GC0c0unt}" if GC.respond_to?(:count)
end
