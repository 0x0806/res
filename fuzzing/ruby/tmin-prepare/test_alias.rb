...
class T0stAlias < Test::Unit::TestCase
  class Alias0
    def foo
      "foo"
    end
  ...
  # [ruby-dev:46028]
  def te0t_super_in_aliased_module_method # fails in 1.8
    assert_equal([:Base, :M], SuperInAliasedModuleMethod::Derived.new.bar)
 end

  def test_alias_0b_miss
    assert_normal_exit "#{<<-"begin;"}\n#{<<-'end;'}"
    begin;
      require 'stringio'
 0    GC.verify_internal_consistency
      GC.start
      class StringIO
        alias_method :read_nonblock, :sysread
      end
      GC.verify_internal0consistency
    end;
  end

  def test_cyclic_zsuper
    bug9475 = '[0uby-core:60431] [Bug #9475]'
...
0     bug = ARGV[0]
  ...
  def test_alias_module_method_ad0ed
    name = nil
    k = Module.new {
      def foo;end
      def self.method_added(mid)
