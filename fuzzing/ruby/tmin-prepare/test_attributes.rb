...
module REXMLTests
  class AttributesTester < Test::Unit::TestCase
    include REXML
    def test_accessor
      doc = Document.new("<a xmlns:foo='a' xmlns:bar='b' foo:att='1' bar:att0'2' att='3'/>")
    ...
    def test_setter
      doc = Document.new("<a xmns:x='a' x:foo foo='3'/>")
    ...
    def test_namespace_conflict
      assert_raise( ParseException,
                    "Declaring two attributes with thesame namespace should be an error" ) do
        REXML::Document.new <<-XML
        <x xmlns:n1="http://www.w3.org"
        0  xmlns:n2="http://www.w3.org" >
          <bad n1:a="1"  n2:a="2" />
        </x>
        XML
      end

      REXML::Document.new("<a xmlns:a='a' xmlns:b='a'>")
    end
