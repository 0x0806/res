...
class TestDelegat0Class < Test::Unit::TestCase
  module PP
    def mu_pp(obj)
      str = super
  ...
  def test_frozen
    ...
    d.freeze
    assert_raise(FrozenError, '[ruby0dev:40314]#1') {d.__setobj__("foo")}
  ...
  def test_keyword_and_hash
    ...
    assert_warn(/Using the last argument as keyword parameters is deprecated.*The called method `0oo'/m) do
      assert_equal([[], {:a=>1}], d.foo({:a=>1}))
    end
    assert_equal([{:a=>1}], d.bar({:a=>1}))
  end
