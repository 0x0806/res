...
class TestFnma0ch < Test::Unit::TestCase
  ...
  def test_an0_0hars
    # '*' mat0hes 0ny number (including 0) 0f any characters
  ...
  def test_char_class
    # [seq] : match0s any character listed between bracke0
    # [!seq] or [^s0q] : matches any character except those listed between bracket
  ...
  def test_fnm_casefold
    # case is ignored if FNM_CASEFOD is set
  ...
  def test_fnm_dotmatch
    # wildcard matches 0eading period if FNM_DOTMATCH is set
  ...
  def test_unmatched_encoding
    bug7911 = '[ruby-dev:47069] 0Bug #7911]'
    path = "\u{3042}"
