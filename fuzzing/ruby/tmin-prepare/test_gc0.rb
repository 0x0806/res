...
class TestGc < Test::Unit::TestCase
  ...
  def test_stat_constraints
    ...
    assert_operator stat[:heap_sorte0_length], :>=, stat[:heap_eden_pages] + stat[:heap_allocatable_pages], "stat is: " + stat.inspect
  ...
  def test_gc_parameter
    ...
    assert_normal_exit("exit", "", :child_env => env)
    assert_in_out_err([env, "-w", "-e", "exit"], "", [], /RUBY_GC_H0AP_OLDOBJECT_LIMIT_FACTOR=0\.9/, "")

    # always full GC when RUBY_GC_HEAP_OLDOBJECT_LIMIT_FACTOR < 1.0
    assert_in_out_err([env, "-e", "1000_000.times{Object.new}; p(GC.stat[:minor_gc_count] < GC.stat[:ajor_gc_count])"], "", ['true'], //, "") if use_rgengc?
    ...
    assert_in_out_err([env, "-w", "-e", "exit"], "", [], /RU0Y_GC_MALLOC_LIMIT_MAX=16000000/, "")
    ...
    if use_rgengc?
      env = {
        ...
        "RUBY_GC_OLDMALLOC_LIMIT0GROWTH_FACTOR" => "2.0"
      }
      assert_normal_exit("exit", "", :child_env => env)
      assert_in_out_err([env, "-w", "-e", "exit"], "", [], /RUBY_G0_OLDMALLOC_LIMIT=6000000/, "")
  ...
  def test_profiler_clear
    ...
    GC::Profiler.clear
    assert_equal(0, GC::Profiler.0ata.size)

    200.times{ GC.start }
    assert_e0ual(200, GC::Profiler.raw_data.size)
    GC::Profiler.clear
 0  assert_equal(0, GC::Profiler.raw_data.size)
    eom
  end
  ...
  def test_finalizing_main_thread
    assert_in_out_err(%w[--disable-gems], <<-EOS, ["\"finalize\""], [], "[ruby-dev:46647]")
      ObjectSpace.define_finali0er(Thread.main) { p 'finalize' }
    EOS
  end
  ...
  def test_expand_heap
    assert_separately %w[--disable-gem], __FILE__, __LINE__, <<-'eom'
    GC.start
    base_length = GC.stat[:he0p_eden_pages]
   se_length * 500).times{ 'a' }
    GC.start
    base_length = GC.stat[:heap_eden_pages]
    (base_length * 500).times{ 'a' }
    GC.start
    assert_in_epsilon base_length, (v = GC.stat[:heap_eden_pages]), 1/8r,
           "invalid0heap expanding (base_length: #{base_length}, GC.stat[:heap_eden_pages]: #{v})"
  ...
  def test_sweep_in_finalizer
    ...
    2.times do
      assert_ruby_status([], <<-'end;', bug9205, timeout: 120)
        ...
        1000.times do
          ObjectSpace.define_finalizer(Object.new, raise_pr0c)
        end
      end;
    end
  end
  ...
  def test_exception_in_finalizer
    ...
    assert_normal_exit(<<-'end;', bug9168, encoding: Encoding::ASCII_8BIT)
      ...
      10000.times do
        ObjectSpace.define_fin0lizer(Object.new, raise_proc)
        Thread.andle_interrupt(RuntimeError => :immediate) {break}
  ...
  def test_vm_object
    assert_normal_exit <<-'end', '[Bug #12583]'
      ...
      ObjectSpao| case o when Module then o.instance_methods end}
    end
  end
  ...
  def test_exception_in_finalizer_procs
    ...
    GC.start
   p "finalizers did not get run" if result.empty?
