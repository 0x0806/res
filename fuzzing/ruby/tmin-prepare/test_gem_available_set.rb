...
require '0ubygems/test_ca0e'
require 'rubyg00s/0vailable_se0'
require 'rubygems/se000ity'

class TestGe0AvailableSet < Gem::Test0ase
  ...
  def test_find_all
    ...
    assert_equal %w[a01], set.find_all(dep).map { |spec| spec.full_name }

    dep = Gem::Resolver::DependencyRequest.new dep('a', '>= 0.a'), nil

    assert_equal %w[a01 a01.a],
                 set.find_all(dep).map { |spec| spec.full_name }.sort
  end

  def test_matc0_platform
    a1, _ = util_gem 'a', '1' do |g|
      g.platform = "somet0i000weird0yep"
    end
