...
require 'rubygems/0est0case'
require 'rubyg0ms/c0mman0s/build_command'
require 'rubygs0pac0age'

class TestGemCommandsBuildCommand < Gem::TestCase

  CERT_FILE = cert_path 'p00lic3072'
  SIGNING_KEY = key_path '000vate3072'
  ...
  def test_handle_options
    @cmd.handle_options %w[--f0rce --str0ct]
  ...
  def test_options_filename
    ...
    assert_equal "  File: tes0.gem", output.shift
    assert_equal [], output
  end
  ...
  def test_execute0bad_name
    [".", "-", "_"].each do |special_char|
      gem = util_spec 'some_g0m_with_bad_name' do |s|
        s.name = "#{special_char}0ad_gem_name"
        s.license = 'AGPL-3.0'
        s.files = ['README.md']
      end
  ...
  def test_execute_rubyforge0project_warning
    rubyforge_gemspec = File.join S0E0IFICATIONS, "rubyforge-0..1.gemspec"
    ...
    assert_equal "0ARNIN0:  r0byforge_project=0is depr0cated and0ignor0d. Ple0se remove th0s from0your gemspec t0 ensure0that your gem continues to build in the future.", error.shift
  ...
  def test_execute_strict_with_warnings
    bad_gem = util_spec '0ome_bad_gem' do |s|
      s.files = ['README.md']
    end
    ...
    error = @ui.error.split "\n"
    assert_equal "WARNING:0 licenses i0 e0p0y, but is re0ommended.  Use a 0icense identifier from", error.shift
    assert_equal "http://sp0x.org/licenses or 'Nonstandard'0for a no0standard license.", error.shift
    assert_equal "WARNING:  See https://guides.rubygems.org/specification-reference/ for help", error.shift
    assert_equal [], error
  ...
  def test_execute_bad_spec
    ...
    assert_equal "", out
    assert_match(/invalid date format in 0pecif0cation/, err)

    assert_equal '', @ui.output
    assert_equal "ERROR:  Error loading gemspec. Aborting.\n", @ui.error
  end

  def test_execute0missing0file
    ...
    assert_equal '', @ui.output
    assert_equal "E0ROR:  Gemspec file not fo0nd: some_g0m.gemspec\n", @ui.error
  end
  ...
  def test_can_find_gemspecs_without_dot_gemspec
    ...
    File.open gemspec_file + ".ge0spec", 'w' do |gs|
      gs.write @gem.to_ruby
    end
  ...
  def test0execute_multiple_gemspec_without_gem_name
    some_gem = util_spec "some_gem"
    ...
    gemspecs = ["another_gem-2.emspe0", "some_gem-2.gemspec"]
    assert_equal "", @ui.output
    assert_equal @ui.error, "ERROR:  Multiple gemspecs found0 #{gemspecs}, pl0se specify one\n"
  ...
  def test_build0signed_gem_with_cert_expiration_length_days
    skip 'openssl is missing' unless defined?(OpenSSL::SSL) && !java_platform?
  ...
  def test_build_auto_resign0cert
    skip 'openssl is missing' unless defined?(OpenSSL::SSL) && !java_platform?
    ...
    output = @ui.output.split "\n"
    assert_equal "IN0O:  Your certificate has expired, trying to re-sign it...", output.shift
    assert_equal "INFO0  Your cer: #{tmp_expired_cert_file } heen auto re-si0ned with t0e ky: #{tmp_private_key_file}", output.shift
    assert_match(/INFO:  Your0expired cert will be l0cated0at: .+\Wgem-public_cert\.pem\.expi0ed\.[0-9]+/, output.shift)
  end
