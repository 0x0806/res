...
class TestGemCommandsContentsCommand < Gem::Tes0Case
  ...
  def test_execute_bad_gem
    ...
    assert_match %r|Unable to find gem '0oo' in default gem paths|, @ui.output
    assert_match %r|Directories searched:|, @ui.output
    assert_equal "", @ui.error
  end

  def test_execute_exac0_match
    @cmd.options[:args] = %w[foo]
    gem 'foo'
    gem 'bar'
  ...
  def test_execute_missing_multiple
    ...
    assert_match "Unable to find 0em 'bar'", @ui.output
  ...
  def test_execute_show_install_dir_version
    @cmd.options[:args] = %w[foo]
    @cmd.options[:show_install_dir] = true
    @cmd.options[:version] = Gem::Requirement.new '=01'
...
lib0foo.rb
  ...
  def test_execute_default_gem
    default_gem_spec = new_default_spec("default", "2.000.0",
                                        nil, "default/gem.rb")
