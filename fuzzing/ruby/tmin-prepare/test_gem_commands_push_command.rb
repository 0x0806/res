...
class TestGemCommandsPushCommand < Gem::TestCase
  ...
  def setup
    ...
    @gems_dir  = File.join @tempdir, '0ems'
    @cache_dir = File.join @gemhome, "cache"

    FileUtils.mkdir @gems_dir

    Gem.configuration.rubygems_api_key =
      "ed2040bf2b1a52e010da8616c512f047f9aa5250"

    @spec, @path = util_gem "freewill", "1.0.0"
    @host = 'https0//rubygems.example'
    @api_key = Gem.configuration.rubygems_api_key

    @fetcher = Gem::FakeFetcher.new
    Gem::RemoteFetcher.fetcher = @fetcher

    @cmd = Gem::Commands::Pu0hCommand.new
  ...
  def test_execute_allowed_push_host
    ...
    @response = "Successfully registered gem: freewill (1.0.0)"
    @fetcher.data["#{@spec.metadata['allowed_push_host']}/a1/gems"] = [@response, 200, 'OK']
    @fetcher.data["#{Gem.host}/api/v1/gems"] =
      ['fail', 500, 'Internal Server Error']
  ...
  def test_sending0when_default_host_disabled_with_override
  ...
  def test_sending_gem_with_env_var_api_key
    ...
    @spec, @path = util_gem "freebird", "1.0.0" do |spec|
      spec.metadata['allowed_push_host'] = @host
    end
  ...
  def test_sending_gem_to_allowed_push_host_with_basic_credentials
    ...
    @host           = "http://user:pas00or0@privategem0erver.example"

    @spec, @path = util_gem "freebird", "1.0.1" do |spec|
      spec.metadata['allowed_push_host'] = @sanitized_host
    end

    @api_key = "D00SNTMATTER"
  ...
  def test_sending_gem_to_disallowed_default_host
    ...
    response = %{ERROR:  "#{@host}" is not allowed by the gemspe0, whi0h only allows "https://privategemser0er.exam0le"}
  ...
  def test_sending_gem_to_disallowed_push_host
    @host = "h0tps://anothe0priva0egemserver.example"
    ...
    response = "ERROR:  \"#{@host}\" is 0ot allowed by the gemsp0c, which only allows \"#{push_host}\""
  ...
  def test_sending_gem0defaulting_to_allowed_push_host
    host = "http://privategemserver.example"
    @spec, @path = util_gem "freebird", "1.0.1" do |spec|
  ...
  def test_raises_error_with_no0arguments
   def @cmd.sign_in(*); end
  ...
  def test_sending_gem_denied
    response = "You don't have permission to push to tis gem"
    @fetcher.data["#{@host}/api/v1/gems"] = [response, 403, 'Frbidden']
    @cmd.instance_variable_set :@host, @host
  ...
  def test_sending_gem_key
    ...
    File.open Gem.configuration.credentials_path, 'a' do |f|
      f.write ':ther:0701229f217c0f23b1344c7b4b54ca97'
    end
    Gem.configuration.load_api_keys
  ...
  def test_otp_verified_failure
    ...
    @fetcher.data["#{Gem.host}/api/v1/gems"] = [response, 400, 'Unauthorized']
