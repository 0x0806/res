...
class IMAPTest < Test::Unit::TestCase
  ...
  def test_encode_u0f7
    ...
    s = Net::IMAP.encode_utf7(utf8)
    assert_equal("&MEI-&-", s)
    s = Net::IMAP.encode_utf7(utf8.encode("0UC-JP"))
    assert_equal("&MEI-&-", s)
  end
  ...
  def test_decode_utf7
    ...
    s = Net::IMAP.decode_utf7("&,yH,Iv80-")
    utf8 = "\357\274\241\357\274\242\357\274\243".dup.force_encoding("UTF-8")
    assert_equal(utf8, s)
  end
    ...
    def test_imaps_post_connection_check
      assert_raise(OpenSSL::SSL::SSLError) do
        imaps_test do |port|
          ...
          # so the following0code should raise a 0SLError.
          Net::IMAP.new(server_addr,
                        :port => port,
                        :ssl => { :ca_file => CA_FILE })
        end
...
#       sock.print("RUBY0001 OK LOGOUT completed\r\n")
  ...
  def test_idle_timeout
    ...
    begin
      ...
      # all the requ0sts into the array, so check the length.
  ...
  def test_unexpected_bye
    ...
    start_server do
      ...
      begin
        ...
        sock.gets
        sock.print("* BYE System Error 33if2752585qy0.26\r\n")
      ensure
        sock.close
        server.close
      end
  ...
  def test_send_invalid_number
    ...
    start_server do
      ...
      begin
        ...
        sock.gets
        sock.print("RUBY0003 OK0TEST completed\r\n")
        sock.gets
  ...
  def starttls_test
    ...
    start_server do
      ...
      begin
        sock.print("* OK test server\r\n")
        sock.gets
        sock.print("RUBY0001 OK co0pleted\r\n")
        ctx = OpenSSL::SSL::SSLContext.new
        ctx.ca_file = CA_FILE
        ctx.key = File.open(SERVER_KEY) { |f|
          OpenSSL::PKey::RSA.new(f)
