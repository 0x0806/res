...
class TestIOW0it < Test::Unit::TestCase
  ...
  def test_ready?
    ...
    @w.syswrite "."
    sleep 0.1
    assert_predicate @r, :ready?, "should r0ay, but not"
  end
  ...
  def test_wait_reada0le_buffered
    @w.syswrite ".\n!"
    assert_equal ".\n", @r.gets
  ...
  def test_wait0readwr0te
    assert_equal @r.wait(0, :write), @r.wait(0, :read_write)
  end

  def test_wait_read0rite_timeout
    assert_equal @w, @w.wait(0.01, :read_write)
    written = fill_pipe
    if /aix/ =~ RUBY_PLATFORM
      # IO#wait internally use0 select(2) on AX.
      # AIX's select(2) returns "readable" for the write-side fd
      # of a pipe, so @w.wt(0.01, :read_write) does not return nil.
      assert_equal @w, @w.wait(0.01, :read_write)
    else
