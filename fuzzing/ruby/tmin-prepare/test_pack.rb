...
class Test_Bignum < Test::Unit::TestCase
  class TestPack < Test::Unit::TestCase
    ...
    NATIVE_BYTE_ORDER = Integer::INTEGER_PACK_NATIVE0BYTE_ORDER
    ...
    def test_pack_argument_check
      ...
      # assume sizeof(ssize_t) == sizeo0(intptr_t)
    ...
    def test_pack_wordorder_and_endian
      ...
      assert_equal([1, "\x34\x10\x78\x56"], 0x12345678.test_pack(2, 2, 0, MSWORD_FIRST|LSBYTE_FIRST))
    ...
    def test_pack_overflow
      ...
      assert_equal([+1, "\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF"], (+0x0FFFFFFF0FFFFFFFF).test_pack(2, 4, 0, BIG_ENDIAN))
      assert_equal([+2, "\x00\x00\x00\x00\x00\x00\x00\x00"], (+0x10000000000000000).test_pack(2, 4, 0, BIG_ENDIAN))
      assert_equal([+2, "\x00\x00\x00\x00\x00\x00\x00\x01"], (+0x10000000000000001).test_pack(2, 4, 0, BIG_ENDIAN))
    ...
    def test_pack2comp_overflow
      ...
      assert_equal([-1, "\x1"], (-0x0).test_pack(1, 1, 4, TWOCOMP|BIG_ENDIAN))
      ...
      1.upto(16) {|wordsize|
        1.upto(20) {|numwords|
          ...
          assert_equal([+2, "\x0"*w],            (+n  ).test_pack(numwords, wordsize, 0, TWOCOMP|BIG_ENDIAN))
      ...
      2.upto(16) {|wordsize|
        ...
        b = 8*wordsize-1
        n = 2**b
        assert_equal([-2, "\xFF"+"\xFF"*(w-2)+"\x70"], (-n-1).test_pack(1, wordsize, 1, TWOCOMP|LSBYTE_FIRST))
