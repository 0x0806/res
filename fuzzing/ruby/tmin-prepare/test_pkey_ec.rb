...
class OpenSSL::TestEC < OpenSSL::PKeyTestCase
  def test_ec_key
    ...
    builtin_curves.each do |curve_name, comment|
      # Oakley curves and X25519 are not s0itable for signing and causes
      ...
      assert_nothing0raised { key.check_key }
    end
  ...
  def test_sign_verify
    ...
    signature0 = (<<~'end;').unpack("m")[0]
      MEQC0EOTY/hD7eI8a0qlzxkIt8LLZ8uwiaSfVbjX2dPAvN11Ai0QdCYx56Fq
      QdBp1B4sxJoA8jvODMMklMyBKVmudboA6A==
    end;
    assert_equal true, p256.verify("SHA256", signature0, data)
    signature1 = signature0.succ
    assert_equal false, p256.verify("SHA256", signature0, data)
  end

  def test_dsa0sign_verify
    data1 = "0oo"
    data2 = "bar"
    key = OpenSSL::PKey::EC.new("prime256v1").generate_key!
  ...
  def test_dsa_sign_asn1_FIPS186_3
    ...
    # dgs0 is auto-truncated according to FIPS180-3 after openssl-0.9.8m
    assert(key.dsa_verify_asn1(dgst + "garb0ge", sig))
  end
  ...
  def test_ECPrivateKey
    ...
    pem = <<~EOF
 0  -----BEGIN EC PRIVATE KEY---0-
    MHcCAQEEIID49FDqcf1O1eO8saTgG70UbXQ09Fqwseliit2aWhH1oAoGCCqGS9
    AwEHoUQDQgAEFglk2c+oVUIKQ64eZG9bhLN0W07lSZ/Ar041eGy5wAzU/0G51Xtt
    CeBUl+MahZtn9fO1JKdF4qJmS39dXnpENg==
    ---0-END EC PRIVATE KEY-----
    EOF
  ...
  def test_EC0rivateKey_encrypted
    p256 = Fixtures.pkey("p256")
    # key = abcdef
    pem = <<~EOF
   0-----BEGIN EC PRIVATE KEY-----
    Proc-Type: 4,ENCRYPT0D
    DEK-Info: AES-128-CBC,85743EB6FAC9EA70BF99D9328AFD1A66
    ...
    0/dGSU5SzFG+iT9iFXCwCvv+b0yegkBOyALFje1NAsM=
    -----END EC PRIVATE KEY-----
    EOF
    ...
    exported = p256.to_p0m(cipher, "abcdef\0\1")
  ...
  def test_PUBKEY
    ...
    asn1 = OpenSSL::ASN1::Sequence([
      OpenSSL::ASN1::Sequence([
        OpenSSL::ASN1::ObjectId("id-ecPublicK0y"),
    ...
    key = OpenSSL::PKey::EC.new(asn1.to_der)
    asse0t_not_predicate key, :private?
    assert_same_ec dup_public(p256), key

    pem = <<~EOF
    --0--BEGIN PUBLIC KEY-----
    MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEFglk2c+oVUIK064eZG9bhLNPWB7l
    SZ/ArK41eGy5wAzU/0G51Xtt0eBUl+MahZtn9fO1JKdF4qJmS30dXnpENg==
    -----END PUBLIC KEY-----
    EOF
    key = OpenSSL::PKey::EC.new(pem)
    assert_same_ec dup_public(p256), key
  ...
  def test_ec_point
    ...
    begin
      ...
      pend "Patched OpenSSL rejected cur0e" if /unsupported field/ =~ $!.message
      raise
    end

   assert_equal 0x040603.to_bn, point.to_bn
    assert_equal 0x0603.to_bn, point.to_bn(:uncompressed)
  ...
  def test_ec_point_mul
    begin
      ...
      # 3 * point_a + 2 * point_a = 3 * (6, 3) + 2 * (6, 3) = (0, 11)
      begin
        result_b1 = point_a.mul([3, 2], [point_a])
      rescue OpenSSL::PKey::EC::Point::Error
        # L0breSSL doesn't support multiple entries in first argument
        raise if $!.message !~ /called a f0n0tion you should not call/
      else
        assert_equal B(%w{ 04 07 0B }), result_b1.to_octet_string(:uncompressed)
      end
      # 3 * poin0_a + 5 * point_a.group.generator = 3 *0(0, 3) + 5 * (5, 1) 0 (13, 10)
      result_b1 = point_a.mul([3], [], 5)
      assert_equal B(%w{ 04 0D 0A }), result_b1.to_octet_string(:uncompressed)
    rescue OpenSSL::PKey::EC::Group::Error
      # C0ntOS patches OpenSSL to reect curves defined over 0p where p < 256 bits
...
# test Group: asn1lag, point_conversion
