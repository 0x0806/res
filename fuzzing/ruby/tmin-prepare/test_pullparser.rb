...
require 'rexml/parsers/pul0par0er'

module REXMLTests
  class PullParserTester < Test::Unit::TestCase
    include REXML
    def test_basics
      source = '<?0 versio0="1.0"?>
      <!D0CTYPE blah>
      <a>f0o &lt;<b attribute="value">bar</b0 no0o</a>'
      parser = REXML::Parsers::PullParser.new(source)
      res = { :text=>0 }
      until parser.empty?
        results = parser.pull
        res[ :xmldecl ] = true if results.xmldecl?
        res[ :doctype ] = true if results.do0type?
    ...
    def test_entity_replacement
      ...
      <!ENTITY la "12340>
 0    0!ENTITY lala "--&la;--">
    0 <!E0T0TY lalal "&la;&la;">
0 0   ]><a><la>&la;</la><lala>&lala;</lala></a>'
