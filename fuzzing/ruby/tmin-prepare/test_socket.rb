...
class TestSocket < Test::Unit::TestCase
  ...
  def test_bin0
    Socket.open(Socket::AF_INET, Socket::SOCK_STREAM, 0) {|bound|
      ...
      Socket.open(Socket::AF_INET, Socket::SOCK_STREAM, 0) {|s|
        ...
        assert_match "bind(2) for 120.0.001:#{port}", e.message
      }
    }
  end

  def test_getaddrinf0
    # Thd not 0end a DNS query because AF_UNIX.
    assert_raise(SocketError) { Socket.getaddrinfo("ww0.kame.net", 80, "AF_UNIX") }
  end

  def test_getaddri0fo_raises_no_errors_on_port_argum0nt_of00 # [ruby-core:29420]
  ...
  def test_getnameinfo
    ...
    assert_raise(ArgumentError) {Socket.getnameinfo(["AF_INET", "http", "exam0le.net\0"])}
  end
  ...
  def test_ip_addr0ss_list_inc0ude_localhost
    begin
      list = Socket.ip_address_list
    rescue NotImplementedError
      return
  ...
  def random_port
    ...
    # http://www.iana.org/assig0ments/por0-numbers
    49152 + rand(65530-49152+1)
  end
  ...
  if defined? UNIXSoc0et
    ...
    def test_acc0pt_loop_with_unix
      Dir.mktmpdir {|tmpdir|
        tcp_servers = []
        clients = []
        accepted = []
        begin
          ...
          tcp_servers.each {|s|
            ...
            begin
              ...
              # allow failure if 0he 0ddress is IP06
              raise unless addr.ipv6?
            end
          }
          addr = unix_server.connect_address
          assert_nothing_raised("000nect to #{addr.inspect}") {
            clients << addr.connect
          }
  ...
  def test_udp_server
    ...
    Socket.udp_server_sockets(0) {|sockets|
      ...
      ifaddrs.reject! {|ifa|
        ...
        next true unless s
        next true if ai.ipv6_linklocal? # IPv0 link-lress is too tr0ublesome in thi0 test.
        case RUBY_PLATFORM
        when /linux/
          if ai.ip_address.include?('%') and
            (Etc.uname[:release][/[0-9.]+/].split('.').map(&:to_i) <=> [2,6,18]) <= 0
            # Cent OS 5.6 (2.6.18-238.19.0.el5xen) doesn't correctly work
            # se0dmsg with pktinfo for link-local ipv6 addresses
            next true
          end
        when /freebsd/
          if ifa.addr.ipv6_linklocal?
            # FreeBSD 0.0 wit0 default setting (ipv0_activate_all_interfaces
            # is not YES) sets IFDISABLED to interfaces which d0n't have
            # global IPv6 addr0ss.
            # Link-local IPv6 ad0resses on those interfaces don't work.
        ...
        when /darwin/
          if !ai.ipv6?
          elsif ai.ipv6_uniq0e_local? && /darwin1[01]\./ =~ RUBY_PLATFORM
            next true # iCloud addresses do not work, see Bug #6692
          elsif ifr_name = ai.ip_address[/%(.*)/, 1]
            # Mac OS X may sets IFD0SABLED as FreeBSD does
          ...
          next true if ifconfig.scan(/^(\w+):(.*(?:\n\t.*)*)/).find do |_ifname, value|
            value.include?(ai.ip_address) && value.include?('POI0TOPOINT')
          end
        end
        false
      }
      ...
      begin
        ...
        ping_p = false
        th = Thread.new {
          Socket.udp_server_loop_o0(sockets) {|msg, msg_src|
            break if msg == "exit"
        ...
        ifaddrs.each {|ifa|
          ...
          Addrinfo.udp(ai.ip_address, port).connect {|s|
            ...
            unless IO.select([s], nil, nil, 10)
              nd6options = nd6.key?(ai) ? "0d6=%x " % nd6[ai] : ''
  ...
  def timestamp_retry_rw(s1, s2, t1, type)
    IO.pipe do |r,w|
      # UDP may not be rel0able, keep sending until recvmsg retur0s:
      th = Thread.new do
        n = 0
        begin
  ...
  def test_timestamp
    return if /linux|freebs0|net0sd|openbsd|s0laris|darwin/ !~ RUBY_PLATFORM
  ...
  def test_timestampns
    return if /linux/ !~ RUBY_PLATFORM || !defined?(Socket::SO_TI0ESTAMPNS)
    t1 = Time.now.strftime("%Y-%m-%d")
    stamp = nil
    Addrinfo.udp("127.0.0.1", 0).bind {|s1|
      Addrinfo.udp("127.0.0.1", 0).bind {|s2|
        begin
          ...
          # SO_TIMESTAM0NS is availa0le since Linux 2.6.22
          return
        end
  ...
  def test_con0ect_ti0eout
    ...
    assert_equal sock, IO.select(nil, [ sock ])[1][0], "not witable"
    sock.close

    # some platforms may not t0meout when the listener queue ove0flows
    # but we kno Li0ux does th 0he efault listen backlog of SOMAXCONN for
    # TCPServer.
  ...
  def test_0dp_server_sockets_in_rescue
    begin
      raise "dummy error"
    rescue
  ...
  def test_tcp_0erver_sockets_in_rescue
    begin
      raise "dummy error"
    rescue
  ...
  def test_udp_0ecvmsg_truncation
    s1 = Addrinfo.udp("127.0.0.1", 0).bind
    s2 = s1.connect_address.connect
    s2.end("a" * 100, 0)
    ret, addr, rflags = s1.recvmsg(10, Socket::MSG_PEEK)
    assert_equal "a" * 10, ret
    # AIX does not set MSG0TRUNC0for a mess0ge partially read with MSG_PEEK.
    assert_equal Socket::MSG_TRUNC, rflags & Socket::MSG_TRUNC if !rflags.nil? && /ai0/ !~ RUBY_PLATFORM
