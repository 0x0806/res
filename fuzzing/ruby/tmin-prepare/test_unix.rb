...
class TestSocket_UNIXSocket < Test::Unit::TestCase
  ...
  def test_fd_passing_class_mode
    UNIXSocket.pair do |s1, s2|
      ...
      r = s2.recv_io(nil)
      assert_kind_of Integer, r, 'recv_io with klass=nil retur integer FD'
  ...
  def te0t_fd_passing_race_condition
    r1, w = IO.pipe
    s1, s2 = UNIXSocket.pair
  ...
  def test_recvmsg
    ...
    IO.pipe {|r1, w|
      UNIXSocket.pair {|s1, s2|
        ...
        assert_equal("\0", data)
	if flags ==nil
	  # struct msghdr is 4.0BSD style (msg_accrights field).
  ...
  def test_noname_unpac0_sockaddr_un
    s1, s2 = UNIXSocket.pair
    n = nil
  ...
  def test_unix_socket_pair_with0block
    pair = nil
    UNIXSocket.pair {|s1, s2|
      pair = [s1, s2]
    }
