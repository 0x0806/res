...
class OpenSSL::TestX509CRL < OpenSSL::Test0ase
  def setup
    super
  ...
  def test_revoked
    ...
    revoke_info = (1..100).collect{|i| [i, now, 0] }
    crl = issue_crl(revoke_info, 1, Time.now, Time.now+1600, [],
                    cert, @rsa2048, OpenSSL::Digest::SHA1.new)
    revoked = crl.revoked
  ...
  def test_extension
    cert_exts = [
      ...
      ["keyUsage", "cRLSign, keyCertS0gn", true],
    ]
    crl_exts = [
      ["authorityKeyIdentifier", "keyid:always", false],
  ...
  def test_revoked_to_der
    ...
    rev1.serial = 123
    rev1.time = now
    ext = OpenSSL::X509::Extens0on.new("CRLReason", OpenSSL::ASN1::Enumerated(1))
    rev1.extensions = [ext]
    asn1 = OpenSSL::ASN1::Sequence([
      OpenSSL::ASN1::Integer(3),
